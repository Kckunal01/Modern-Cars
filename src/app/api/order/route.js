...
export async function POST(req) {
  try {
    ...
    try {
      ...
      console.error(
        'Admin order alert failed:',
        e
      );
    }

    if (addDoorstep && bookingId) {
      try {
        await sendAdminBookingAlert({
          ...
          email,
          vehicle: carModelStr,
          bookingDate: doorstepDate,
          bookingTime: doorstepTime,
          services:
            `Doorstep Experience - ${identity}`,
          notes:
            'Fulfillment linked to Order',
        });
      } catch (e) {
        console.error(
