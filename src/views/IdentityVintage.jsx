...
export default function IdentityVintage() {
  const router = useRouter();
  const { setSelectedIdentity } = useAppContext();
  const slides = ['/Assets/V1.png', '/Assets/V2.png', '/Assets/V3.png', '/Assets/V4.png'];
  ...
  const handleCheckout = () => {
    ...
    const selection = { id: 'vintage', name: 'VINTAGE', brand, model, year, img: '/Assets/Vintage.png' };
  ...
  return (
    <div className="identity-page-wrapper theme-vintage" style={{ paddingTop: '72px' }}>
      
      <section className="id-hero-carousel-3up">
        <button className="carousel-nav-btn prev" onClick={scrollLeft}>‹</button>
        <div className="carousel-track" ref={trackRef}>
          {slides.map((src, index) => (
            <img key={index} src={src} alt={`Vintage ${index + 1}`} className="carousel-slide" />
          ))}
        </div>
      ...
      <section className="id-cabin-section">
        <div className="id-cabin-content">
          <h3>THE OLD MONEY CABIN</h3>
          <h2>Luxury that<br/>doesn't chase trends.<br/>It outlives them.</h2>
          <div style={{ width: '40px', height: '2px', background: 'var(--border)', marginBottom: '32px' }} />
          <p>Warm tones. Rich textures.<br/>A cabin that reflects taste, heritage and quiet confidence.</p>
          <div className="id-cabin-tags">
            HERITAGE <span>•</span> TASTE <span>•</span> TIMELESS
          </div>
        </div>
        <div className="id-cabin-image">
          <img src="/Assets/Vintage.png" alt="Vintage Cabin" />
        </div>
      </section>
      ...
      <section className="id-materials-section">
        ...
        <h2>Premium Materials. Timeless Experience.</h2>
        <div className="id-materials-grid">
          <div className="id-material-card">
            <img src="/Assets/VM1.png" alt="Vintage Material 1" />
          </div>
          <div className="id-material-card">
            <img src="/Assets/VM2.png" alt="Vintage Material 2" />
          </div>
          <div className="id-material-card">
            <img src="/Assets/VM3.png" alt="Vintage Material 3" />
          </div>
        </div>
      </section>
